module hp_library::msg_utils {
    ...
    public fun version(bytes: &vector<u8>): u8 {
        hacks::to_u8(extract_from_bytes(bytes, 0, 1))
    }

    public fun nonce(bytes: &vector<u8>): u32 {
        hacks::to_u32(
            extract_from_bytes_reversed(bytes, 1, 5)
        )
    }

    public fun origin_domain(bytes: &vector<u8>): u32 {
        hacks::to_u32(
            extract_from_bytes_reversed(bytes, 5, 9)
        )
    }
    ...
    public fun dest_domain(bytes: &vector<u8>): u32 {
        hacks::to_u32(
            extract_from_bytes_reversed(bytes, 41, 45)
        )
    }

    public fun recipient(bytes: &vector<u8>): address {
        hacks::to_address(extract_from_bytes(bytes, 45, 77))
    }
    ...
    public fun extract_module_name_from_body(body_bytes: &vector<u8>): String {
        let module_name_length = hacks::to_u32(
            extract_from_bytes_reversed(body_bytes, 0, 4)
        );
        let module_name_bytes = extract_from_bytes(
            body_bytes,
            4,
            ((4 + module_name_length) as u64)
        );
